    ...
    if(!empty($_SESSION['cart'])){
        foreach($_POST['remove_code'] as $key){
            ...
            if(isset($_SESSION['cart'][$key])){
                ...
                $restore_qty = $_SESSION['cart'][$key]['quantity'];
                    ...
                    <div class="table-responsive">
                        ...
                        if(!empty($_SESSION['cart'])){
                            ...
                            foreach($_SESSION['cart'] as $cart_key => $cart_item){
                                if(isset($cart_item['product_id'])){
                                    ...
                                    // Handle old cart structure (without size)
                                    $product_ids[] = explode('_', $cart_key)[0];
                                }
                            }
                        ...
                        ?>
                            <table class="table table-bordered">
                                ...
                                $products = array();
                                while($product_row = mysqli_fetch_array($query)){
                                ...
                                foreach($_SESSION['cart'] as $cart_key => $cart_item){
                                    ...
                                    if(isset($cart_item['product_id'])){
                                        // New cart structure with size
                                        $product_id = $cart_item['product_id'];
                                        $size = isset($cart_item['size']) ? $cart_item['size'] : 'N/A';
                                        $quantity = $cart_item['quantity'];
                                    } else {
                                        // Old cart structure (backward compatibility)
                                        $product_id = explode('_', $cart_key)[0];
                                        $size = 'N/A';
                                        $quantity = $cart_item['quantity'];
                                    }
                                    ...
                                    if(isset($products[$product_id])){
                                        ...
                                        $subtotal = ($quantity * $product['productPrice']) + $product['shippingCharge'];
                                        $grand_total += $subtotal;
                                        $total_items += $quantity;
                                        $total_shipping += $product['shippingCharge'];
                                        
                                        array_push($pdtid, $product_id);
                                ?>
                                    <tr>
                                        <td class="romove-item">
                                            <input type="checkbox" name="remove_code[]" value="<?php echo htmlentities($cart_key);?>" />
                                        </td>
                                        <td class="cart-image">
                                        ...
                                        <td class="cart-product-name-info">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-sm-8">
                                                    <?php 
                                                    $rt=mysqli_query($con,"select * from productreviews where productId='".$product['id']."'");
                                        ...
                                        <td class="cart-product-size">
                                            <span class="size-info"><?php echo htmlentities($size); ?></span>
                                        </td>
                                        <td class="cart-product-quantity">
                                            <div class="quant-input">
                                                ...
                                                <input type="text" value="<?php echo $quantity; ?>" name="quantity[<?php echo $cart_key; ?>]">
                                            </div>
                                        </td>
                                        <td class="cart-product-sub-total">
                                            <span class="cart-sub-total-price">Rs. <?php echo number_format($product['productPrice'], 2); ?></span>
                                        </td>
                                        <td class="cart-product-sub-total">
                                            <span class="cart-sub-total-price">Rs. <?php echo number_format($product['shippingCharge'], 2); ?></span>
                                        </td>
                                        <td class="cart-product-grand-total">
                                            <span class="cart-grand-total-price">Rs. <?php echo number_format($subtotal, 2); ?></span>
                                        </td>
                                    </tr>
                                <?php 
                                    }
                    ...
                    <div class="col-md-4 col-sm-12 cart-shopping-total">
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th>
                                        <span class="estimate-title">Order Summary</span>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td>
                                        <div class="cart-summary">
                                            ...
                                            <div class="summary-row">
                                                <span>Subtotal:</span>
                                                <span>Rs. <?php echo number_format($grand_total - $total_shipping, 2); ?></span>
                                            </div>
                                            <div class="summary-row">
                                                <span>Shipping Charges:</span>
                                                <span>Rs. <?php echo number_format($total_shipping, 2); ?></span>
                                            </div>
                                        ...
                                        <div class="cart-checkout-btn pull-right">
                                            <button type="submit" name="ordersubmit" class="btn btn-primary btn-upper">PROCEED TO CHECKOUT</button>
                                        </div>
                                    </td>
                                </tr>
                            </tbody><!-- /tbody -->
                        ...
                        <?php 
                        // Store grand total in session for checkout
                        $_SESSION['tp'] = $grand_total;
                        ?>
                    </div>
                <?php 
                } else {
                    echo "<div class='col-md-12'><h3 class='text-center'>Your shopping cart is empty</h3><p class='text-center'><a href='index.php' class='btn btn-primary'>Continue Shopping</a></p></div>";
                }
                ?>
            </div>
        </div> 
